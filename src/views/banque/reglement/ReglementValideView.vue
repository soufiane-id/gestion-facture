<template>
    ...
    <template v-slot:cell(action)="operationBancaire">   
      ...
      <div v-if="operationBancaire.item.statutOperation == 'VALIDE'">
        <a href="javascript:;" v-b-modal="'detailsReglement'" @click="detailOperation(operationBancaire.item.codeOperation)" >Voir Détails</a>
      </div>
    </template>
...
import DetailReglementModule from "./DetailReglementModal";
...
export default {
    ...
    methods: {
    afficherOperationsValides(){
      http
        .get("/operationsBancairesValides")
        .then(response => {
    ...
    detailOperation(codeOperation) {
      this.$store.commit('updateCodeOperation', codeOperation);
    },
    ...
    filtredOperations(){
      ...
      let operationsFiltred = this.operationsBancaire.filter((operation) => {
        return String(operation.montantCredit).match(this.$store.state.filterOperations)
              ...
              || String(operation.codeOperation).match(this.$store.state.filterOperations)
  ...
  created() {
    this.afficherOperationsValides();
  }
}
</script>