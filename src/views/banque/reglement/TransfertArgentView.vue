...
export default {
  ...
  methods: {
      afficherTransfertsArgent(){
      http
        .get("/operationsTransfertBancaire")
        .then(response => {
  ...
  computed: {
    ...
    filtredOperations: function(){
      this.currentPage = 1;
      console.log('this.filterOperations', this.$store.state.filterOperations)
