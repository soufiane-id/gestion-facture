...
export default {
  ...
  methods: {
    monSuperTraitement(jsonBody) {
      ...
      if (
        !JSON.stringify(jsonBody).includes("Date") ||
      ...
      jsonBody = jsonBody.filter((operation) => {
        if (
          ...
          (!operation.montantDebit && !operation.montantCredit)
        ) {
          return false;
        }
        return true;
      ...
      for (let i in jsonBody) {
        ...
        const dateParts = jsonBody[i].dateOperation.split("/");
        const regex = /^[0-9]{2}$/;
        ...
        /* remplacer les virgules par des points dans le montantDebit et montantDebit */
        if (jsonBody[i].montantDebit.includes(",")) {
          jsonBody[i].montantDebit = jsonBody[i].montantDebit.replace(",", ".");
        }
