<template>
  <div>
    <b-row>
      <Formulaire
        :typePersonne="routeTypePersonne"
    ...
    <div class="card-tools">
      ...
      <button
        ...
        @click="modifierFacture(null)"
        v-b-modal="'formulaireAjoutEC'"
        style="float:right"
      >
    ...
    >
      <template v-slot:cell(montantFacture)="data">
        ...
        <b-badge
          v-if="data.item.operations && data.item.operations.length > 0"
          variant="primary"
          title="Num Opération"
        >{{data.item.operations}}</b-badge>
      </template>
      ...
      <template v-slot:custom-foot>
        <tr>
          ...
          <td
            v-if="routeTypePersonne.toLowerCase() == 'client'"
            class="bg-success text-white text-center"
          >{{totalRAPClient}}</td>
          <td
            v-if="routeTypePersonne.toLowerCase() == 'fournisseur'"
            class="bg-success text-white text-center"
          >{{totalRAPFrs}}</td>
          <td
            v-if="routeTypePersonne.toLowerCase() == 'client'"
            class="bg-success text-white text-center"
          >{{totalFacturesClient}}</td>
          <td
            v-if="routeTypePersonne.toLowerCase() == 'fournisseur'"
            class="bg-success text-white text-center"
          >{{totalFacturesFrs}}</td>
      ...
      <template v-slot:cell(action)="xxx">
        <b-col cols="6" sm="4" md="2" xl class="mb-3 mb-xl-0">
          <a
            v-if="xxx.item.resteAPayer === xxx.item.montantFacture"
            @click="modifierFacture(xxx.item)"
            class="edit"
            title="Modifier"
            data-toggle="tooltip"
  ...
  watch: {
    ...
    selectedOptionAffichage: function (newValue, oldValue) {
      if (this.routeTypePersonne.toLowerCase() === "client") {
        newValue == 1
          ? this.afficherEcheancesClientNonReglees()
          : this.afficherEcheancesClientReglees();
      } else if (this.routeTypePersonne.toLowerCase() === "fournisseur") {
        newValue == 1
          ? this.afficherEcheancesFrsNonReglees()
          : this.afficherEcheancesFrsReglees();
      }
    },
  },
  methods: {
    recupererEcheanciers() {
      if (this.routeTypePersonne.toLowerCase() === "client") {
        this.selectedOptionAffichage == 1
          ? this.afficherEcheancesClientNonReglees()
          : this.afficherEcheancesClientReglees();
      } else if (this.routeTypePersonne.toLowerCase() === "fournisseur") {
        this.selectedOptionAffichage == 1
    ...
    refreshTable(ech) {
      ...
      const societeAdded = this.mapSociete.find(
        (soc) => soc.idSociete === idSoc
      );
  ...
  computed: {
    ...
    filtredEcheanciers: function () {
      ...
      let societesFiltres = this.echeanciers.filter((ech) => {
        return ech.societe.nomSociete
          .toLowerCase()
          .match(this.filterSearchSociete.toLowerCase());
      });

      let clientsFiltres = societesFiltres.filter((ech) => {
        return (
          ech.personne.nomPersonne
            .toLowerCase()
            .match(this.filterSearchClient.toLowerCase()) ||
          ech.numeroDocument
            .toLowerCase()
            .match(this.filterSearchClient.toLowerCase())
        );
      ...
      if (this.startDate !== null && this.endDate !== null) {
        ...
        clientsFiltres = clientsFiltres.filter(function (element) {
          return (
            vm.convertDate(vm.startDate) <=
              vm.stringToDate(element.dateFacture) &&
            vm.stringToDate(element.dateFacture) <= vm.convertDate(vm.endDate)
          );
        });
      }
      ...
      if (this.routeTypePersonne.toLowerCase() === "client") {
        if (clientsFiltres.length == 0) {
          ...
          this.totalFacturesClient = clientsFiltres
            .map((o) => o.montantFacture)
            .reduce((a, c) => a + c)
            .toFixed(2);
          this.totalRAPClient = clientsFiltres
            .map((o) => o.resteAPayer)
            .reduce((a, c) => a + c)
            .toFixed(2);
        }
        ...
        if (clientsFiltres.length == 0) {
          ...
          this.totalFacturesFrs = clientsFiltres
            .map((o) => o.montantFacture)
            .reduce((a, c) => a + c)
            .toFixed(2);
          this.totalRAPFrs = clientsFiltres
            .map((o) => o.resteAPayer)
            .reduce((a, c) => a + c)
            .toFixed(2);
        }
